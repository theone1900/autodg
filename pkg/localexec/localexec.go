...
func InitStdListener(cfg service.SourceConfig,dbname string,oracle_sid string) (string error){
	cmds := fmt.Sprintf(`echo "
LISTENER =
(DESCRIPTION_LIST =
(DESCRIPTION =
...
(SID_DESC =
	...
	(ORACLE_HOME = %s)
	(SID_NAME = %s)
) )" >> %s/network/admin/listener.ora`,cfg.StandbyHostIps[0],dbname,cfg.StandbyOracleHome,oracle_sid,cfg.StandbyGridHome)
	fmt.Println("[init listener]",cmds)
...
func StartRmanDuplicate(cfg service.SourceConfig) (string error){
	...
	db_unique_name := fmt.Sprintf(`'%s'`,cfg.OracleUniqname+"unqi")
